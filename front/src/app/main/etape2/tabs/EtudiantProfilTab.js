...
//import Countries from 'app/main/Countries';

const EtudiantProfilTab = () =>
{
    
    const dispatch = useDispatch();
    const register = useSelector(({auth}) => auth.etape2);
    const [isFormValid, setIsFormValid] = React.useState(false);
    const formRef = useRef(null);

    useEffect(() => {
        if ( register.error && (register.error.nom || register.error.prenom || register.error.dateNaissance || register.error.ecole || register.error.pays) )
        {
    ...
    {
        dispatch(authActions.submitRegisterEtudiant(model));
        console.log(model);
    }
...
export default EtudiantProfilTab;
